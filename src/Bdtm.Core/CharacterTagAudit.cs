...
namespace Bdtm.Core
{
    ...
    public static class CharacterTagAuditResponseParser
    {
            ...
            string triggerWord)
        {
            ...
            try
            {
                ...
                if (rootNode is not JsonObject root)
                    throw new CharacterTagAuditResponseException("The model response is not valid character tag JSON.");
                ...
                foreach (JsonNode? token in tags)
                {
                    ...
                    if (!TryParseDecision(GetString(obj, "decision"), out CharacterTagDecision decision))
                        throw new CharacterTagAuditResponseException("Response contains an invalid decision for: " + tag);
                    CharacterTagCategory category = ParseCategory(GetString(obj, "category"));
                    string replacementTag = GetString(obj, "replacement_tag")?.Trim() ?? string.Empty;
                    ...
                    bool includeInPrompt = GetBool(obj, "include_in_prompt") ?? false;
                    int promptOrder = GetInt(obj, "prompt_order") ?? int.MaxValue;
        ...
        {
            if (!obj.TryGetPropertyValue(name, out JsonNode? node) || node is null || node is JsonValue value && value.GetValueKind() == JsonValueKind.Null)
                return null;
        ...
        private static bool? GetBool(JsonObject obj, string name)
        {
            ...
            if (node is JsonValue jv)
            {
                if (jv.TryGetValue<bool>(out bool b))
                    return b;
                if (jv.TryGetValue<string>(out string? s) && bool.TryParse(s, out bool parsed))
                    return parsed;
            }
            return null;
        }
        ...
        private static int? GetInt(JsonObject obj, string name)
        {
            ...
            if (node is JsonValue jv)
            {
                if (jv.TryGetValue<int>(out int i))
                    return i;
                if (jv.TryGetValue<long>(out long l) && l >= int.MinValue && l <= int.MaxValue)
                    return (int)l;
                if (jv.TryGetValue<string>(out string? s) && int.TryParse(s, out int parsed))
                    return parsed;
            }
            return null;
        }
    ...
    public sealed class CharacterTagAuditService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            string inventoryJson = JsonSerializer.Serialize(auditedInventory.Tags);
        ...
        private static string BuildVisualPrompt(IReadOnlyList<CharacterTagAuditItem> initial)
        {
            return "Review the preliminary tag decisions against the attached reference image. "
                ...
                + "replacement_tag.\nPreliminary: "
                + JsonSerializer.Serialize(initial.Select(item => new
                {
                    tag = item.Tag,
            ...
            CancellationToken cancellationToken)
        {
            ...
            catch (CharacterTagAuditResponseException original)
            {
                var repair = new CharacterTagModelRequest
                {
                    ...
                    UserPrompt = "Expected tags: " + JsonSerializer.Serialize(inventory.Tags.Select(item => item.Tag))
                        + "\nMalformed response:\n" + response.Result
                };
